...
<div class="pointer-events-none absolute inset-0 hidden dark:block">
    <div class="absolute -top-28 left-1/4 h-80 w-72 bg-green-500/5 blur-3xl"></div>
    <div class="absolute top-0 right-1/4 h-80 w-64 bg-emerald-400/5 blur-3xl"></div>
</div>

<div class="relative mb-8 flex flex-col md:flex-row justify-between items-start md:items-center">
    <div>
        <h1 class="text-3xl font-extrabold text-gray-900 dark:text-slate-100 mb-2">Our <span class="text-green-600 dark:text-[#69da58]">Products</span></h1>
        <p class="text-gray-600 dark:text-slate-300">Fresh groceries delivered straight to your door.</p>
    </div>
</div>

<div class="mb-8 rounded-xl border border-gray-100 dark:border-slate-800 bg-white dark:bg-[#0f1726] p-5 shadow-sm dark:shadow-none">
        ...
        <div>
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                <input
                    ...
                    class="w-full rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-[#111a2b] text-gray-900 dark:text-slate-100 px-4 py-3 text-sm shadow-sm focus:border-green-500 focus:ring-green-500"
                >
                ...
                @if(!empty($search) || request('category'))
                    <a href="{{ route('products.index') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-300 dark:border-slate-700 px-5 py-3 text-sm font-semibold text-slate-700 dark:text-slate-200 transition hover:bg-gray-50 dark:hover:bg-[#111a2b]">
                        Clear
                    </a>
                @endif
            </div>
...
<div class="relative flex flex-col lg:flex-row gap-8">
    ...
    <div class="w-full lg:w-1/4">
        <div class="bg-white dark:bg-[#0f1726] rounded-xl shadow-sm dark:shadow-none border border-gray-100 dark:border-slate-800 p-6 sticky top-24">
            ...
            @php
                ...
                $renderCategoryIcon = function ($icon) {
                    switch ($icon) {
                        case 'all':
                            return '<svg class="w-4 h-4 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4"/><circle cx="9" cy="19" r="1.5" /><circle cx="17" cy="19" r="1.5" /></svg>';
                        case 'frozen':
            ...
            @endphp
            <div class="space-y-2">
                <a href="{{ route('products.index') }}" class="block px-3 py-2 rounded-md transition {{ !request('category') ? 'bg-green-50 dark:bg-[#162235] text-green-700 dark:text-green-300 font-medium' : 'text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-[#162235]' }}">
                    <span class="mr-2">{!! $renderCategoryIcon('all') !!}</span> All Products
                </a>
                @foreach($categories as $category)
                    <a href="{{ route('products.index', ['category' => $category]) }}" class="block px-3 py-2 rounded-md transition {{ request('category') === $category ? 'bg-green-50 dark:bg-[#162235] text-green-700 dark:text-green-300 font-medium' : 'text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-[#162235]' }}">
    ...
    <div class="w-full lg:w-3/4">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($products as $product)
                <div class="bg-white dark:bg-[#0f1726] border border-gray-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm dark:shadow-none hover:shadow-md dark:hover:border-slate-600 transition flex flex-col">
                    <a href="{{ route('products.show', $product) }}" class="relative h-48 bg-gray-100 dark:bg-[#0d1525] group block overflow-hidden">
                        ...
                        @else
                            <div class="w-full h-full flex items-center justify-center text-gray-400 dark:text-slate-400 group-hover:scale-105 transition duration-300">
                                <svg class="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                </svg>
                            </div>
                        @endif
                        <span class="absolute top-2 right-2 bg-white/90 dark:bg-[#1f7f3a] backdrop-blur text-xs font-bold px-2 py-1 rounded text-gray-700 dark:text-white shadow-sm">
                            {!! $renderCategoryIcon($categoryIcons[$product->category] ?? null) !!} {{ $product->category }}
                        </span>
                    </a>
                    ...
                    <div class="p-4 flex-grow flex flex-col justify-between">
                        ...
                        <div class="flex gap-2 mt-auto">
                            <form action="{{ route('cart.add', $product) }}" method="POST" class="flex-1 add-to-cart-form" data-product-name="{{ $product->name }}">
                                ...
                                <button type="button" class="w-full bg-green-50 dark:bg-[#162235] text-green-700 dark:text-green-300 hover:bg-green-100 dark:hover:bg-[#1c2b44] py-2 px-3 rounded-md text-sm font-semibold transition flex justify-center items-center add-to-cart-btn">
            ...
            @empty
                <div class="col-span-full py-12 text-center bg-gray-50 dark:bg-[#0f1726] rounded-xl border border-gray-100 dark:border-slate-800 p-8">
...
<div id="add-to-cart-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 p-4">
    <div class="w-full max-w-sm rounded-xl bg-white dark:bg-[#0f1726] border border-gray-200 dark:border-slate-800 p-6 shadow-xl">
        ...
        <input id="modal-quantity" type="number" min="1" value="1" class="mt-2 w-full rounded-md border border-gray-300 dark:border-slate-700 bg-white dark:bg-[#18243a] text-gray-900 dark:text-slate-100 px-3 py-2 focus:border-green-500 focus:outline-none focus:ring-1 focus:ring-green-500">

        <div class="mt-5 flex justify-end gap-2">
            <button id="add-to-cart-cancel" type="button" class="rounded-md border border-gray-300 dark:border-slate-700 px-4 py-2 text-sm font-medium text-gray-700 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-[#18243a]">Cancel</button>
