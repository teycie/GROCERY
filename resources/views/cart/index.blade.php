...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8 flex flex-col md:flex-row items-center justify-between">
        ...
        <a href="{{ route('products.index') }}" class="mt-4 md:mt-0 text-green-600 font-semibold hover:text-green-700 flex items-center transition">
    ...
    @if(empty($items))
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            <div class="mx-auto flex items-center justify-center h-24 w-24 rounded-full bg-gray-50 mb-6 text-gray-400">
                <svg class="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
            </div>
            <h2 class="text-2xl font-bold text-gray-800 mb-2">Cart is empty</h2>
            <p class="text-gray-500 mb-6 text-lg">Looks like you haven't added anything yet.</p>
    ...
    @else
        <form action="{{ route('cart.checkout') }}" method="POST" id="checkoutForm" class="flex flex-col lg:flex-row gap-8">
            @csrf
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                        <label class="flex items-center space-x-3 cursor-pointer group">
                            ...
                            <span class="text-sm font-bold text-gray-700 group-hover:text-gray-900 transition">Select All/None</span>
                        </label>
                    </div>
                    ...
                    <ul class="divide-y divide-gray-100">
                        @foreach($items as $item)
                            ...
                            @endphp
                            <li class="p-6 hover:bg-gray-50/50 transition duration-150">
                                    ...
                                    <div class="ml-6 flex flex-1 flex-col">
                                        ...
                                        <div>
                                            <div class="flex justify-between text-base font-bold text-gray-900">
                                                <h3><a href="{{ route('products.show', $item['product']) }}" class="hover:text-green-600 transition">{{ $item['product']->name }}</a></h3>
                                                <p class="ml-4 tabular-nums">?{{ number_format($item['line_total'], 2) }}</p>
                                            </div>
                                            <p class="mt-1 text-sm text-gray-500">?{{ number_format($unitPrice, 2) }} each</p>
                                        </div>
                                        <!-- Quantity Form Actions -->
                                        <div class="flex flex-1 items-end justify-between text-sm mt-4">
                                            <p class="text-gray-500 bg-gray-100 px-3 py-1 rounded-full font-medium">Qty: {{ $item['quantity'] }}</p>

                                            <div class="flex">
                                                <button type="button" onclick="document.getElementById('remove-form-{{ $item['product_id'] }}').submit()" class="font-bold text-red-500 hover:text-red-600 hover:bg-red-50 px-3 py-1 rounded-md transition flex items-center">
            ...
            <div class="w-full lg:w-1/3">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-24">
                    <h2 class="text-lg font-bold text-gray-900 flex items-center border-b pb-4 mb-4">
                    ...
                    <div class="space-y-3 mb-6">
                        ...
                        <div class="flex justify-between font-bold text-lg border-t pt-4">
                            <p>Total Estimated</p>
                            <p class="tabular-nums text-green-600" id="totalPreview">?0.00</p>
                        </div>
                    </div>
                    ...
                    <button type="button" onclick="document.getElementById('remove-selected-form').submit()" class="w-full bg-red-50 text-red-600 border border-red-100 rounded-lg py-3 px-4 text-sm font-bold hover:bg-red-100 transition focus:outline-none focus:ring-2 focus:ring-red-500 flex justify-center items-center">
                        <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
                        Remove Selected Items
                    </button>
                    <p class="text-xs text-gray-400 mt-4 text-center">Shipping & taxes calculated at checkout.</p>
                </div>
            </div>
        </form>
